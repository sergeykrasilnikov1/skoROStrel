...
class LaneFollower(Node):
    ...
    def process_image(self, cv_image):
        ...
        if self.follow_yellow_only:
            ...
            if yellow_pixels < 2000:  # Порог для обнаружения зеленого света
                ...
                time.sleep(1.0)
                self.set_active_node('parking 50')
        else:
    ...
    def find_center_offset(self, cv_image):
        ...
        if self.follow_yellow_only:
            ...
            # self.get_logger().info(f"yellow_center: {yellow_center} white_center {white_center}")
    ...
    def send_control_command(self, center_offset):
        ...
        angular_speed = -float(center_offset) / self.turn_speed

        # Параметры для управления скоростью
        max_linear_speed = 0.3  # Максимальная линейная скорость на прямых
        ...
        twist.linear.x = max_linear_speed - (normalized_angular ** slowdown_factor) * (max_linear_speed - min_linear_speed)
